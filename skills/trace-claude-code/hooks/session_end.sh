...
if [ -n "$TURN_SPAN_ID" ] && [ -f "$CONV_FILE" ]; then
    ...
    create_llm_span() {
        ...
        [ -z "$output" ] && return
        local span_id=$(generate_uuid | sed 's/-//g' | head -c 16)
        local span_start="${start_time:-$(get_time_nanos)}"
        local span_end="${end_time:-$(get_time_nanos)}"
        local input_json=$(echo "$history" | jq -c '.' | jq -Rs '.')
        local output_json=$(jq -n --arg c "$output" '[{role: "assistant", content: $c}]' | jq -c '.' | jq -Rs '.')
        local attrs=$(build_otlp_attributes "$(jq -n \
        ...
        local span=$(build_otlp_span "$TRACE_ID" "$span_id" "$TURN_SPAN_ID" "${model:-anthropic.messages.create}" "llm" "$span_start" "$span_end" "$attrs" 20)
        local duration_ms=$(( (span_end - span_start) / 1000000 ))
        insert_span "$PROJECT_ID" "$span" >/dev/null && { LLM_CALLS=$((LLM_CALLS + 1)); debug "LLM span: $model (${duration_ms}ms)"; } || true
    }
    ...
    create_tool_span() {
        ...
        case "$tool_name" in
            Read|Write|Edit) local f=$(echo "$tool_input" | jq -r '.file_path // .path // empty' 2>/dev/null); [ -n "$f" ] && span_name="$tool_name: $(basename "$f")" ;;
            Bash|Terminal) local c=$(echo "$tool_input" | jq -r '.command // empty' 2>/dev/null); span_name="Terminal: ${c:-cmd}" ;;
        esac
        local input_json=$(echo "$tool_input" | jq -c '.' 2>/dev/null | jq -Rs '.')
        local output_json=$(echo "$tool_output" | jq -Rs '.')
        local attrs=$(build_otlp_attributes "$(jq -n --arg span_kind "tool" --argjson input "$input_json" --argjson output "$output_json" --arg tool_name "$tool_name" \
            '{"judgment.span_kind": $span_kind, "judgment.input": $input, "judgment.output": $output, "tool_name": $tool_name}')")
        local span=$(build_otlp_span "$TRACE_ID" "$span_id" "$TURN_SPAN_ID" "$span_name" "tool" "$start_time" "$end_time" "$attrs" 20)
    ...
    while IFS= read -r line; do
        ...
        if [ "$MSG_TYPE" = "user" ]; then
            ...
            CONTENT_TYPE=""
            echo "$CONTENT" | jq -e 'type == "array"' >/dev/null 2>&1 && CONTENT_TYPE=$(echo "$CONTENT" | jq -r '.[0].type // empty' 2>/dev/null)
            ...
            if [ "$CONTENT_TYPE" = "tool_result" ]; then
                ...
                while IFS= read -r TOOL_RESULT; do
                    ...
                    if [ -n "$TOOL_USE_RESULT" ] && [ "$TOOL_USE_RESULT" != "null" ] && echo "$TOOL_USE_RESULT" | jq -e 'type == "object"' >/dev/null 2>&1; then
                        ...
                        if [ "$TOOL_TYPE" = "text" ]; then
                            ...
                            [ -n "$FILE_CONTENT" ] && TOOL_OUT="$FILE_CONTENT" || TOOL_OUT=$(echo "$TOOL_USE_RESULT" | jq -r '.text // "completed"')
                        else
                        ...
                        TOOL_OUT=$(echo "$RAW_OUT" | sed 's/^[[:space:]]*[0-9]*→//')
                    fi
                    ...
                    if [ -n "$TOOL_USE_ID" ]; then
                        ...
                        if [ -n "$PENDING" ] && [ "$PENDING" != "null" ]; then
                            ...
                            if [ -n "$P_START" ] && [ "$P_START" != "null" ] && [ "$P_START" -gt 0 ] 2>/dev/null; then
                                ...
                                [ -n "$END_NANOS" ] && [ "$END_NANOS" -gt 0 ] 2>/dev/null && create_tool_span "$P_NAME" "$P_INPUT" "$TOOL_OUT" "$P_START" "$END_NANOS"
                            fi
                ...
                if [ "$CONTENT" != "null" ] && [ -n "$CONTENT" ]; then
                    TXT="$CONTENT"
                    echo "$CONTENT" | jq -e '.' >/dev/null 2>&1 && TXT=$(echo "$CONTENT" | jq -r 'if type == "array" then [.[] | select(.type == "text") | .text] | join("\n") else . end' 2>/dev/null)
            ...
            if echo "$line" | jq -e '.message.content | type == "array"' >/dev/null 2>&1; then
                while IFS= read -r TOOL_USE; do
                    ...
                    [ -n "$TOOL_ID" ] && [ -n "$TOOL_NAME" ] && PENDING_TOOLS=$(echo "$PENDING_TOOLS" | jq --arg id "$TOOL_ID" --arg name "$TOOL_NAME" --arg input "$TOOL_INPUT" --arg start "$(iso_to_nanos "$TIMESTAMP")" \
            ...
            MODEL=$(echo "$line" | jq -r '.message.model // empty' 2>/dev/null); [ -n "$MODEL" ] && CURRENT_MODEL="$MODEL"
            USAGE=$(echo "$line" | jq -c '.message.usage // {}' 2>/dev/null)
            if [ "$USAGE" != "{}" ]; then
                INP=$(echo "$USAGE" | jq -r '.input_tokens // 0'); [ "$INP" != "null" ] && [ "$INP" -gt 0 ] 2>/dev/null && CURRENT_PROMPT_TOKENS=$((CURRENT_PROMPT_TOKENS + INP))
                OUT=$(echo "$USAGE" | jq -r '.output_tokens // 0'); [ "$OUT" != "null" ] && [ "$OUT" -gt 0 ] 2>/dev/null && CURRENT_COMPLETION_TOKENS=$((CURRENT_COMPLETION_TOKENS + OUT))
                CC=$(echo "$USAGE" | jq -r '.cache_creation_input_tokens // 0'); [ "$CC" != "null" ] && [ "$CC" -gt 0 ] 2>/dev/null && CURRENT_CACHE_CREATION=$((CURRENT_CACHE_CREATION + CC))
                CR=$(echo "$USAGE" | jq -r '.cache_read_input_tokens // 0'); [ "$CR" != "null" ] && [ "$CR" -gt 0 ] 2>/dev/null && CURRENT_CACHE_READ=$((CURRENT_CACHE_READ + CR))
            fi
        fi
    done < "$CONV_FILE"
