...
# Fallback-finalize a still-open turn (Stop was killed before finalizing).
# In the normal case Stop already finalized and cleared active_*, so this is
# skipped — but the drain + prune below always run.
if [ -n "$TRACE_ID" ] && [ -n "$ROOT_SPAN_ID" ] && [ -n "$TASK_SPAN_ID" ]; then
...
# Queue drained: all attach jobs that needed this session's blobs have run.
# Remove the session entry, its subagent mappings, and their blob files so
# the state file and blob dir never accumulate across sessions.
